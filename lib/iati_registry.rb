module IATI
    class Registry
        ...
        class Sync
            ...
            def self.calculate_updates
#                 registry = parent.publisher_datasets({:mapped => false}) - (parent.publisher_datasets - Organization.iati_eligible)
                ...
                registry.each do |reg|
                    ...
                    if nam.projects.size > 0
                        ...
                        ngoaidmap_freshest = Project.organizations(nam.id).order('updated_at desc').first.updated_at
                        queue << reg if (ngoaidmap_freshest - registry_freshest) > 0
                    end
                end
                queue
            end
        ...
        class << self
            ...
            def publisher_datasets(options={})
                ...
                results = Rails.cache.fetch("package_search/#{params.to_s}", :compress => true, :expires_in => 1.day) do
