...
SQL
  ...
  def get_profile
    ...
    profile[:countries] = Country.joins([:projects => :primary_organization]).where('organizations.id = ?', self.id)
    profile[:donors] = Donor.joins([:donations => [:project => :primary_organization]]).where('organizations.id = ?', self.id)
